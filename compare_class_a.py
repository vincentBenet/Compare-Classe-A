...
class CompareClassA:
    ...
    def run(self):
        ...
        self.dlg.expression_reference.setExpression("$z")
        self.dlg.expression_result.setExpression("$z")
        ...
        if result:
            auto_offset = self.dlg.auto_offset.isChecked()
            ...
            if auto_offset:
                ...
                def objective_function(offsets):
                    ...
                    pos_result_adjusted = pos_result.copy()
                    pos_result_adjusted[:, 0] += offset_x
                    pos_result_adjusted[:, 1] += offset_y
                    pos_result_adjusted[:, 2] += offset_z
                    dists_xy_test, dists_z_test, _ = compare_curves(pos_reference, pos_result_adjusted)
                    scores_test = compare_pipe_to_pipe(dists_xy_test, dists_z_test)
                    score = -(
                        (scores_test["XY 60% 20cm"] - scores["XY 60% 20cm"]) +
                        (scores_test["XY 90% 40cm"] - scores["XY 90% 40cm"]) +
                        (scores_test["XY 100% 150cm"] - scores["XY 100% 150cm"]) +
                        (scores_test["Z 90% 40cm"] - scores["Z 90% 40cm"]) +
                        (scores_test["Z 100% 70cm"] - scores["Z 100% 70cm"]) +
                    0)
                    return score

                # Initial guess for the offsets
                initial_guess = [0, 0, 0]

                # Set bounds based on max distance to constrain the search space
                max_dist = self.dlg.max_dist.value()
                bounds = [
                    (-max_dist, max_dist),  # bounds for x offset
                    (-max_dist, max_dist),  # bounds for y offset
                    (-max_dist, max_dist)  # bounds for z offset
                ]

                # Run the optimization with bounds to prevent extreme offsets
                result = minimize(
                    objective_function,
                    initial_guess,
                    bounds=bounds,
                    options={'disp': False, 'maxiter': 1000, 'ftol': 1e-5, 'gtol': 1e-5}
                )

                # Get the optimized offsets
                offset_x, offset_y, offset_z = result.x

                pos_result[:, 0] += offset_x
                pos_result[:, 1] += offset_y
                pos_result[:, 2] += offset_z
                dists_xy, dists_z, pos_interp = compare_curves(pos_reference, pos_result, self.dlg.max_dist.value())
                scores = compare_pipe_to_pipe(dists_xy, dists_z)
...
def compare_curves(reference, computed, max_dist=None, step=0.01):
